...
def learn_spot_review_policy(
    ...
    *,
    ...
    # The current bounded Spot learner only derives the negative auto-approve
    # threshold from confirmed-negative rows. Positive and fallback behavior
    # remain explicit fixed policy flags rather than learned policy outputs.
...
def _infer_spot_policy_scope(
    ...
    # Keep the current Spot scope contract intentionally narrow: one-company
    # corpora produce a company-scoped artifact, and mixed corpora fall back
    # to a global artifact.
    company_ids = {
