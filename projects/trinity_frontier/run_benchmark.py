from __future__ import annotations

import argparse
import json

from prepare import ensure_prepared, load_cases
from train import evaluate_case


def run_benchmark() -> dict[str, object]:
    ensure_prepared()
    cases = load_cases()
    scores = [evaluate_case(case) for case in cases]
    metric_value = round(sum(scores) / len(scores), 6)

    return {
        "status": "succeeded",
        "metric_value": metric_value,
        "result_summary": "Trinity frontier benchmark completed against bounded ranking fixtures.",
        "error_message": None,
    }


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--budget-seconds", type=int, required=True)
    parser.add_argument("--run-id", type=int, required=True)
    parser.parse_args()
    print(json.dumps(run_benchmark()))


if __name__ == "__main__":
    main()
